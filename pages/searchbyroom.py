...
class SearchByRoom:
    ...
    def open(self) :
        ...
        search_by_section.click_button_search_section('1.3. Сведения о помещениях, машино-местах')
    ...
    def check_page_is_opened(self):
        ...
        assert page_title == 'Поиск разделов 1.3', 'Не удалось открыть страницу'
...
#получаем из БД количество объектов для ТУ в статусах "Учтен" или "Исключен" по agency_id
    def get_count_of_object_from_db(self, agency_id):
        with ConnectionsDB.conn.cursor(cursor_factory=DictCursor) as cursor:
            ...
            try:
                cursor.execute('SELECT count(*) FROM slice.registry_slice_card_room '
